...
try:
    with open("deployment_metadata.json") as f:
        data = json.load(f)
        remote_agent_engine_id = data.get("remote_agent_engine_id")
...
if remote_agent_engine_id and remote_agent_engine_id != "None":
    ...
    if len(parts) >= 6:
        ...
        logger.warning(f"Invalid remote_agent_engine_id format: {remote_agent_engine_id}")
        # Set defaults or raise error if critical, here defaulting for safety during collection
