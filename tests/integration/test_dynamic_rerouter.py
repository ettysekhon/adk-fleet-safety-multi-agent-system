...
async def run_dynamic_rerouter_tests(mcp_wrapper):
    """Run the dynamic rerouter tests with the given MCP wrapper."""
    # --- Initialise agents ---
    ...
    trip = {
        ...
        or initial_route.get("overview_polyline", {}).get("points", ""),
        "planned_remaining_duration_minutes": initial_route.get("duration_in_traffic_minutes", 200),
    }
    ...
    try:
        if mcp_server_url:
            ...
            async with AsyncExitStack() as stack:
                ...
                await run_dynamic_rerouter_tests(mcp_wrapper)
        else:
